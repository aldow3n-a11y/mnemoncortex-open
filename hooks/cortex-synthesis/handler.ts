...
// LLM extraction config — uses Ollama Cloud (default) or Gemini (fallback)
...
// Importance heuristic based on pattern type
...
function mnemonLink(sourceId: string, targetId: string, type: string, weight: number, meta?: Record<string, string>): void {
  ...
  try {
    ...
    // Links are best-effort, don't fail the pipeline
...
// ─── Core Functions ──────────────────────────────────────────────────────────
interface QueueItem {
  timestamp: string; source: string; pattern: string; content: string;
...
// ─── LLM Extraction (Ollama Cloud → Gemini fallback) ─────────────────────────
async function extractTriples(content: string): Promise<Triple[]> {
  ...
  if (EXTRACTION_BACKEND === "ollama") {
    ...
    console.log("[cortex-synthesis] Ollama extraction failed, falling back to Gemini");
...
async function extractTriplesOllama(content: string): Promise<Triple[]> {
  try {
    ...
    if (!response.ok) {
      console.log(`[cortex-synthesis] Ollama extraction failed: ${response.status}`);
      return [];
    }
...
function observeRegex(content: string, source: string): number {
  ...
  if (captured > 0) { saveQueue(queue); console.log(`[cortex-synthesis] Regex captured ${captured} insights (tags: ${queue.slice(-captured).flatMap(q => q.tags).join(", ")})`); }
  return captured;
}
...
async function observeWithLLM(content: string, source: string): Promise<number> {
  ...
  for (const triple of triples) {
    queue.push({
      timestamp: new Date().toISOString(), source, pattern: "llm-extraction",
      content: `${triple.subject} → ${triple.relation} → ${triple.object}`,
      tags: [triple.category], type: "triple", triple: { subject: triple.subject, relation: triple.relation, object: triple.object, confidence: triple.confidence },
    });
    captured++;
  }
  if (captured > 0) { saveQueue(queue); console.log(`[cortex-synthesis] LLM extracted ${captured} triples (backend: ${EXTRACTION_BACKEND}, categories: ${triples.map(t => t.category).join(", ")})`); }
  return captured;
}
...
function distill() {
  ...
  const mnemonIds: string[] = []; // Track Mnemon IDs for linking
  ...
  for (const item of queue) {
    if (item.type === "triple" && item.triple) {
      ...
      // Write triple to Mnemon as an insight
      ...
      // Write distilled insight to Mnemon
...
const handler = async (event: any) => {
  if (event.type === "message:sent" && event.context?.content) {
    ...
    try { await observeWithLLM(content, "message:sent"); }
    catch (err) { console.log(`[cortex-synthesis] LLM extraction skipped: ${err}`); }
  }
  if (event.type === "command:new" || event.type === "command:reset") { distill(); }
  if (event.type === "gateway:startup") {
    const queue = loadQueue();
    if (queue.length > 0) { console.log(`[cortex-synthesis] Gateway startup: flushing ${queue.length} orphaned items`); distill(); }
  }
};
